...
@OnlyIn(Dist.CLIENT)
public class DwarfSelectScreen extends AbstractContainerScreen<DwarfSelectMenu> {
	private final static HashMap<String, Object> guistate = DwarfSelectMenu.guistate;
	...
	public void init() {
		super.init();
		button_conf = new PlainTextButton(this.leftPos + 16, this.topPos + 27, 46, 20, Component.translatable("gui.simpleraces.dwarf_select.button_conf"), e -> {
			if (true) {
				ModMessages.INSTANCE.sendToServer(new DwarfSelectButtonMessage(0, x, y, z));
				DwarfSelectButtonMessage.handleButtonAction(entity, 0, x, y, z);
			}
		}, this.font);
		guistate.put("button:button_conf", button_conf);
		this.addRenderableWidget(button_conf);
		button_empty = new PlainTextButton(this.leftPos + -7, this.topPos + -14, 25, 20, Component.translatable("gui.simpleraces.dwarf_select.button_empty"), e -> {
			if (true) {
				ModMessages.INSTANCE.sendToServer(new DwarfSelectButtonMessage(1, x, y, z));
				DwarfSelectButtonMessage.handleButtonAction(entity, 1, x, y, z);
			}
		}, this.font);
		guistate.put("button:button_empty", button_empty);
		this.addRenderableWidget(button_empty);
		button_empty1 = new PlainTextButton(this.leftPos + -9, this.topPos + 7, 25, 20, Component.translatable("gui.simpleraces.dwarf_select.button_empty1"), e -> {
			if (true) {
				ModMessages.INSTANCE.sendToServer(new DwarfSelectButtonMessage(2, x, y, z));
				DwarfSelectButtonMessage.handleButtonAction(entity, 2, x, y, z);
			}
		}, this.font);
