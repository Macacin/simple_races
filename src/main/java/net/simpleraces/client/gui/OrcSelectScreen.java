...
public class OrcSelectScreen extends AbstractContainerScreen<OrcSelectMenu> {
	...
	public void render(GuiGraphics guiGraphics, int mouseX, int mouseY, float partialTicks) {
		...
		if (OrcReturnProcedure.execute(world) instanceof LivingEntity livingEntity) {
			InventoryScreen.renderEntityInInventoryFollowsAngle(guiGraphics, this.leftPos + -37, this.topPos + 32, 30, 0f + (float) Math.atan((this.leftPos + -37 - mouseX) / 40.0), (float) Math.atan((this.topPos + -17 - mouseY) / 40.0),
					livingEntity);
		}
		...
		if (mouseX > leftPos + 22 && mouseX < leftPos + 46 && mouseY > topPos + -41 && mouseY < topPos + -17)
			guiGraphics.renderTooltip(font, Component.translatable("gui.simpleraces.orc_select.tooltip_dwarves_are_stout_skilled_craft"), mouseX, mouseY);
		if (mouseX > leftPos + 46 && mouseX < leftPos + 70 && mouseY > topPos + -41 && mouseY < topPos + -17)
			guiGraphics.renderTooltip(font, Component.translatable("gui.simpleraces.orc_select.tooltip_passive_mine_faster_in_dark"), mouseX, mouseY);
	...
	public void init() {
		super.init();
		button_conf = new PlainTextButton(this.leftPos + 15, this.topPos + 27, 46, 20, Component.translatable("gui.simpleraces.orc_select.button_conf"), e -> {
			if (true) {
